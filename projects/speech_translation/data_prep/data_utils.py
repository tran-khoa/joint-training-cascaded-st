...
def extract_fbank_features(
    ...
    _waveform, sample_rate = convert_waveform(waveform, sample_rate, to_mono=True)
...
def obtain_paths_and_lengths(data_root: Path, is_audio=False):
    files = list(data_root.glob("*.npy"))

    paths = {}
    lengths = {}

    for file in tqdm(files):
        utt_id = Path(file.filename).stem
        paths[utt_id] = f"{(data_root / file).as_posix()}"

        with file.open('rb') as f:
            byte_data = f.read()
            assert len(byte_data) > 1
            if is_audio:
                assert is_sf_audio_data(byte_data), utt_id
            else:
                assert is_npy_data(byte_data), utt_id
            if is_audio:
                lengths[utt_id] = sf.info(f).frames
            else:
                lengths[utt_id] = np.load(f).shape[0]
