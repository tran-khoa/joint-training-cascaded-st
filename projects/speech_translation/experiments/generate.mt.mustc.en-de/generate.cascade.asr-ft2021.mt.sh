...
ASR_COMMON=/work/smt4/gao/tran/speech_translation/trained_models/asr.mustc.en-de/fair.ctc.ft2021.lr1e-4.noctc/results_COMMON/generate-tst-COMMON_asr.txt
ASR_HE=/work/smt4/gao/tran/speech_translation/trained_models/asr.mustc.en-de/fair.ctc.ft2021.lr1e-4.noctc/results_HE/generate-tst-HE_asr.txt

echo "Extracting ASR hypotheses..."
cat $ASR_COMMON | grep ^H- | sed s/H-//| sort -h | cut -f3 > $MODEL_PATH/results_CASCADED/asr_COMMON.asr_enc.txt
cat $ASR_HE | grep ^H- | sed s/H-//| sort -h | cut -f3 > $MODEL_PATH/results_CASCADED/asr_HE.asr_enc.txt

echo "spm_decode..."
python3 $USER_DIR/evaluation/spm_decode.py $MODEL_PATH/results_CASCADED/asr_COMMON.asr_enc.txt > $MODEL_PATH/results_CASCADED/asr_COMMON.txt
python3 $USER_DIR/evaluation/spm_decode.py $MODEL_PATH/results_CASCADED/asr_HE.asr_enc.txt > $MODEL_PATH/results_CASCADED/asr_HE.txt
...
python3 $USER_DIR/data_prep/spm_encode.py /work/smt2/vtran/datasets/mt/out/bpe_vocab/parnia.iwslt2020_mt_en-de.lowercase-nopunct.32k.en.model \
  $MODEL_PATH/results_CASCADED/asr_COMMON.txt > $MODEL_PATH/results_CASCADED/asr_COMMON.mt_enc.txt
  python3 $USER_DIR/data_prep/spm_encode.py /work/smt2/vtran/datasets/mt/out/bpe_vocab/parnia.iwslt2020_mt_en-de.lowercase-nopunct.32k.en.model \
  $MODEL_PATH/results_CASCADED/asr_HE.txt > $MODEL_PATH/results_CASCADED/asr_HE.mt_enc.txt

echo "Start MT decoding..."
cat $MODEL_PATH/results_CASCADED/asr_COMMON.mt_enc.txt | \
  python /work/smt2/vtran/fairseq-st/fairseq_cli/interactive.py \
    $DATA_PATH --task custom_translation $EVAL_ARGS $OTHER_ARGS > $MODEL_PATH/results_CASCADED.asr-ft2021/gen_COMMON.txt

cat $MODEL_PATH/results_CASCADED/asr_HE.mt_enc.txt | \
  python /work/smt2/vtran/fairseq-st/fairseq_cli/interactive.py \
    $DATA_PATH --task custom_translation $EVAL_ARGS $OTHER_ARGS > $MODEL_PATH/results_CASCADED.asr-ft2021/gen_HE.txt
