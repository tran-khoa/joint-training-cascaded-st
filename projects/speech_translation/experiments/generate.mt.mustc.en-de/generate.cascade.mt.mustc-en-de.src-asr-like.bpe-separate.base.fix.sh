...
mkdir -p $MODEL_PATH/results_CASCADED
cd $MODEL_PATH
...
ASR_COMMON=/work/smt4/gao/tran/speech_translation/trained_models/asr.mustc.en-de/fair.trafo.ctc/results_COMMON/generate-tst-COMMON_asr.txt
ASR_HE=/work/smt4/gao/tran/speech_translation/trained_models/asr.mustc.en-de/fair.trafo.ctc/results_HE/generate-tst-HE_asr.txt
...
cat $MODEL_PATH/results_CASCADED/asr_COMMON.mt_enc.txt | \
  python /work/smt2/vtran/fairseq-st/fairseq_cli/interactive.py \
    $DATA_PATH --task custom_translation $EVAL_ARGS $OTHER_ARGS > $MODEL_PATH/results_CASCADED/gen_COMMON.txt

cat $MODEL_PATH/results_CASCADED/asr_HE.mt_enc.txt | \
  python /work/smt2/vtran/fairseq-st/fairseq_cli/interactive.py \
    $DATA_PATH --task custom_translation $EVAL_ARGS $OTHER_ARGS > $MODEL_PATH/results_CASCADED/gen_HE.txt
