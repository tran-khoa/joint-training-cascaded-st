...
GROUP_NAME=st.iwslt.en-de

FAIRSEQ_PATH=/work/smt4/gao/tran/speech_translation/fairseq
DATA_PATH=/work/smt2/vtran/datasets/asr/parnia.iwslt.st
...
WANDB_RUN_GROUP=$GROUP_NAME WANDB_NAME=$MODEL_NAME python -m torch.distributed.launch --nproc_per_node=$NUM_GPUS \
  ...
  $DATA_PATH --task custom_speech_to_text --config-yaml $DATA_PATH/config_st.yaml $MODEL_ARGS $TRAIN_ARGS $OTHER_ARGS
