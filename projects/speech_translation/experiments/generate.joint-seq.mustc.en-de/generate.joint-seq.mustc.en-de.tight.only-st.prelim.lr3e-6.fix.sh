...
MODEL_NAME=joint-seq.mustc.en-de.tight.only-st.prelim.lr3e-6.fix
...
do
  ...
  cd $MODEL_PATH
  python $FAIRSEQ_PATH/fairseq_cli/generate.py \
    --task speech_to_text --gen-subset tst-HE_st --pivot-beam $pb  --results-path piv${pb}_seq_tight_nomtnorm_results_HE --model-overrides "{'tight_integrated_decoding_temp':1.5}" $MODEL_ARGS $EVAL_ARGS $OTHER_ARGS
...
do
  ...
  python $FAIRSEQ_PATH/fairseq_cli/generate.py \
    --task speech_to_text --gen-subset tst-COMMON_st --pivot-beam $pb --results-path piv${pb}_tight_seq_nomtnorm_results_COMMON --model-overrides "{'tight_integrated_decoding_temp':-1}" $MODEL_ARGS $EVAL_ARGS $OTHER_ARGS
  cd piv${pb}_seq_nomtnorm_results_COMMON
  eval_COMMON_de generate-tst-COMMON_st.txt

  cd $MODEL_PATH
  python $FAIRSEQ_PATH/fairseq_cli/generate.py \
    --task speech_to_text --gen-subset tst-HE_st --pivot-beam $pb  --results-path piv${pb}_seq_tight_nomtnorm_results_HE --model-overrides "{'tight_integrated_decoding_temp':-1}" $MODEL_ARGS $EVAL_ARGS $OTHER_ARGS
  cd piv${pb}_seq_nomtnorm_results_HE
  eval_HE_de generate-tst-HE_st.txt
done