...
python $FAIRSEQ_PATH/fairseq_cli/generate.py \
  --task speech_to_text --gen-subset tst-COMMON_st --results-path piv4_seq_nonorm_temp${temp}_results_COMMON $MODEL_ARGS $EVAL_ARGS $OTHER_ARGS \
...
cd $MODEL_PATH/piv4_seq_nonorm_temp${temp}_results_COMMON
/u/vtran/bin/eval_COMMON_de generate-tst-COMMON_st.txt

cd $MODEL_PATH
python $FAIRSEQ_PATH/fairseq_cli/generate.py \
  --task speech_to_text --gen-subset tst-HE_st --results-path piv4_seq_nonorm_temp${temp}_results_HE $MODEL_ARGS $EVAL_ARGS $OTHER_ARGS \
  --model-overrides "{'asr_model_conf':'$USER_DIR/experiments/train.joint-seq.mustc.en-de/asr.base.ft.yaml','mt_model_conf':'$USER_DIR/experiments/train.joint-seq.mustc.en-de/mt.base.yaml','tight_integrated':True,'tight_integrated_decoding_temp':$temp}" \

cd $MODEL_PATH/piv4_seq_nonorm_temp${temp}_results_HE
