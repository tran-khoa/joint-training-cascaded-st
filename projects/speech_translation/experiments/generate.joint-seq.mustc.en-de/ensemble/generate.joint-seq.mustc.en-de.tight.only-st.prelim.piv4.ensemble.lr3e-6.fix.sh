...
MODEL_NAME=joint-seq.mustc.en-de.tight.only-st.prelim.piv4.ensemble.lr3e-6.fix
...
--model-overrides {'tight_integrated_decoding_temp':1.5} \
--ensemble-decoding \
...
python $FAIRSEQ_PATH/fairseq_cli/generate.py \
  --task speech_to_text --gen-subset tst-COMMON_st --results-path results_COMMON $MODEL_ARGS $EVAL_ARGS $OTHER_ARGS
cd results_COMMON
eval_COMMON_de generate-tst-COMMON_st.txt

cd $MODEL_PATH
python $FAIRSEQ_PATH/fairseq_cli/generate.py \
  --task speech_to_text --gen-subset tst-HE_st --results-path results_HE $MODEL_ARGS $EVAL_ARGS $OTHER_ARGS
cd results_HE
