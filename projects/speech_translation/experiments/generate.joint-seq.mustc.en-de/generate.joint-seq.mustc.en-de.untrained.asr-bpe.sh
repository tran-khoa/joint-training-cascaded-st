...
python $FAIRSEQ_PATH/fairseq_cli/generate.py \
  --task speech_to_text --gen-subset tst-COMMON_st --results-path piv1_results_COMMON $MODEL_ARGS $EVAL_ARGS $OTHER_ARGS \
  --pivot-beam 1 --batch-size 16
cd $MODEL_PATH/piv1_results_COMMON
/u/vtran/bin/eval_COMMON_de generate-tst-COMMON_st.txt
cd $MODEL_PATH

python $FAIRSEQ_PATH/fairseq_cli/generate.py \
  --task speech_to_text --gen-subset tst-HE_st --results-path piv1_results_HE $MODEL_ARGS $EVAL_ARGS $OTHER_ARGS \
  --pivot-beam 1 --batch-size 16
cd $MODEL_PATH/piv1_results_HE
/u/vtran/bin/eval_HE_de generate-tst-HE_st.txt
cd $MODEL_PATH

python $FAIRSEQ_PATH/fairseq_cli/generate.py \
  --task speech_to_text --gen-subset tst-COMMON_st --results-path piv4_results_COMMON $MODEL_ARGS $EVAL_ARGS $OTHER_ARGS \
  --pivot-beam 4 --batch-size 8
cd $MODEL_PATH/piv4_results_COMMON
/u/vtran/bin/eval_COMMON_de generate-tst-COMMON_st.txt
cd $MODEL_PATH

python $FAIRSEQ_PATH/fairseq_cli/generate.py \
  --task speech_to_text --gen-subset tst-HE_st --results-path piv4_results_HE $MODEL_ARGS $EVAL_ARGS $OTHER_ARGS \
  --pivot-beam 4 --batch-size 8
cd $MODEL_PATH/piv4_results_HE
/u/vtran/bin/eval_HE_de generate-tst-HE_st.txt
cd $MODEL_PATH