...
python $FAIRSEQ_PATH/fairseq_cli/generate.py \
  --task speech_to_text --gen-subset tst-COMMON_st --results-path nonorm_ensemble_results_COMMON $MODEL_ARGS $EVAL_ARGS $OTHER_ARGS

python $FAIRSEQ_PATH/fairseq_cli/generate.py \
  --task speech_to_text --gen-subset tst-HE_st --results-path nonorm_ensemble_results_HE $MODEL_ARGS $EVAL_ARGS $OTHER_ARGS
