...
do
  ...
  python $FAIRSEQ_PATH/fairseq_cli/generate.py \
    --task speech_to_text --gen-subset tst-COMMON_st --pivot-beam $pb --results-path piv${pb}_tok_nomtnorm_results_COMMON $MODEL_ARGS $EVAL_ARGS $OTHER_ARGS
  cd piv${pb}_tok_nomtnorm_results_COMMON
  eval_COMMON_de generate-tst-COMMON_st.txt

  cd $MODEL_PATH
  python $FAIRSEQ_PATH/fairseq_cli/generate.py \
    --task speech_to_text --gen-subset tst-HE_st --pivot-beam $pb --results-path piv${pb}_tok_nomtnorm_results_HE $MODEL_ARGS $EVAL_ARGS $OTHER_ARGS
  cd piv${pb}_tok_nomtnorm_results_HE
