...
MODEL_NAME=joint-seq.mustc.en-de.base.only-st.ensemble.piv1
...
do
  cd $MODEL_PATH
  echo "tok pivot-beam $pb"
  python $FAIRSEQ_PATH/fairseq_cli/generate.py \
    --task speech_to_text --gen-subset tst-COMMON_st --pivot-beam $pb --results-path piv${pb}_tok_tight_nomtnorm_results_COMMON --ensemble-decoding --ensemble-decoding-weight-asr $MODEL_ARGS $EVAL_ARGS $OTHER_ARGS
  cd piv${pb}_tok_tight_nomtnorm_results_COMMON
  eval_COMMON_de generate-tst-COMMON_st.txt

  cd $MODEL_PATH
  python $FAIRSEQ_PATH/fairseq_cli/generate.py \
    --task speech_to_text --gen-subset tst-HE_st --pivot-beam $pb --results-path piv${pb}_tok_tight_nomtnorm_results_HE --ensemble-decoding --ensemble-decoding-weight-asr $MODEL_ARGS $EVAL_ARGS $OTHER_ARGS
  cd piv${pb}_tok_tight_nomtnorm_results_HE
...
do
  cd $MODEL_PATH
  echo "seq pivot-beam $pb"
  python $FAIRSEQ_PATH/fairseq_cli/generate.py \
    --task speech_to_text --gen-subset tst-COMMON_st --pivot-beam $pb --results-path piv${pb}_seq_tight_nomtnorm_results_COMMON $MODEL_ARGS $EVAL_ARGS $OTHER_ARGS
  cd piv${pb}_seq_tight_nomtnorm_results_COMMON
  eval_COMMON_de generate-tst-COMMON_st.txt

  cd $MODEL_PATH
  python $FAIRSEQ_PATH/fairseq_cli/generate.py \
    --task speech_to_text --gen-subset tst-HE_st --pivot-beam $pb --results-path piv${pb}_seq_tight_nomtnorm_results_HE $MODEL_ARGS $EVAL_ARGS $OTHER_ARGS
  cd piv${pb}_seq_tight_nomtnorm_results_HE
  eval_HE_de generate-tst-HE_st.txt
done