...
MODEL_NAME=fair.trafo.ctc.lr3e-4
...
echo $DATA_PATH --task custom_speech_to_text --config-yaml $DATA_PATH/config_st.yaml $MODEL_ARGS $TRAIN_ARGS $OTHER_ARGS
echo '-------------------------'

python $FAIRSEQ_PATH/fairseq_cli/generate.py \
  $DATA_PATH --task speech_to_text --gen-subset tst-COMMON_st --results-path results_COMMON --config-yaml $DATA_PATH/config_st.yaml $MODEL_ARGS $EVAL_ARGS $OTHER_ARGS

python $FAIRSEQ_PATH/fairseq_cli/generate.py \
  $DATA_PATH --task speech_to_text --gen-subset tst-HE_st --results-path results_HE --config-yaml $DATA_PATH/config_st.yaml $MODEL_ARGS $EVAL_ARGS $OTHER_ARGS
