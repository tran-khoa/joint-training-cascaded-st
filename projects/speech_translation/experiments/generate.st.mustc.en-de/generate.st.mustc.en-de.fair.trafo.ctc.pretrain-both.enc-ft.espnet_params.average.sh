...
MODEL_NAME=fair.trafo.ctc.pretrain-both.enc-ft.espnet_params
...
--log-format simple \
--log-interval 100 \
--path $MODEL_PATH/checkpoints/average.pt \
--beam 12 \
--nbest 1 \
...
python $FAIRSEQ_PATH/fairseq_cli/generate.py \
  $DATA_PATH --task speech_to_text --gen-subset tst-COMMON_st --results-path average_results_COMMON --config-yaml $DATA_PATH/config_st.mt-bpe.yaml $MODEL_ARGS $EVAL_ARGS $OTHER_ARGS

python $FAIRSEQ_PATH/fairseq_cli/generate.py \
  $DATA_PATH --task speech_to_text --gen-subset tst-HE_st --results-path average_results_HE --config-yaml $DATA_PATH/config_st.mt-bpe.yaml $MODEL_ARGS $EVAL_ARGS $OTHER_ARGS
