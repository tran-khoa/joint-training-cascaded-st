...
python $FAIRSEQ_PATH/fairseq_cli/generate.py \
  $DATA_PATH --task speech_to_text --gen-subset tst-COMMON_asr --results-path results_nocmvn_COMMON_12best --config-yaml $DATA_PATH/config_asr.i6approx.yaml $MODEL_ARGS $EVAL_ARGS $OTHER_ARGS

python $FAIRSEQ_PATH/fairseq_cli/generate.py \
  $DATA_PATH --task speech_to_text --gen-subset tst-HE_asr --results-path results_nocmvn_HE_12best --config-yaml $DATA_PATH/config_asr.i6approx.yaml $MODEL_ARGS $EVAL_ARGS $OTHER_ARGS
