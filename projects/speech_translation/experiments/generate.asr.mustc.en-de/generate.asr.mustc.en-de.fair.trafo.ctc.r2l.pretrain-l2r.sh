...
python $FAIRSEQ_PATH/fairseq_cli/generate.py \
  $DATA_PATH --task custom_speech_to_text --gen-subset tst-COMMON_asr --results-path results_COMMON --config-yaml $DATA_PATH/config_asr.yaml $MODEL_ARGS $EVAL_ARGS $OTHER_ARGS

python $FAIRSEQ_PATH/fairseq_cli/generate.py \
  $DATA_PATH --task custom_speech_to_text --gen-subset tst-HE_asr --results-path results_HE --config-yaml $DATA_PATH/config_asr.yaml $MODEL_ARGS $EVAL_ARGS $OTHER_ARGS
