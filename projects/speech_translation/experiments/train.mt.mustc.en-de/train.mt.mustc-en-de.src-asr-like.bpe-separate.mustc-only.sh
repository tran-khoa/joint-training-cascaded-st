...
MODEL_NAME=src-asr-like.bpe-separate.mustc-only
...
if [ $NUM_GPUS -eq 1 ]; then
  ...
  WANDB_RUN_GROUP=$GROUP_NAME WANDB_NAME=$MODEL_NAME python -m torch.distributed.launch --nproc_per_node=$NUM_GPUS --master_port 29505 \
