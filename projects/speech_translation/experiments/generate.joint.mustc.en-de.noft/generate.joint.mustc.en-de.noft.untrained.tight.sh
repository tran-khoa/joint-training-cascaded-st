...
MODEL_NAME=joint.mustc.en-de.noft.untrained.asr_bpe
...
--model-overrides {'asr_model_conf':'$USER_DIR/experiments/train.joint.mustc.noft/asr.base.noft.yaml','mt_model_conf':'$USER_DIR/experiments/train.joint-seq.mustc.en-de/mt.base.yaml','tight_integrated':True,'tight_integrated_decoding_temp':1.5} \
...
do
  ...
  python $FAIRSEQ_PATH/fairseq_cli/generate.py \
    --task speech_to_text --gen-subset tst-COMMON_st --pivot-beam $pb --results-path piv${pb}_seq_tight_results_COMMON $MODEL_ARGS $EVAL_ARGS $OTHER_ARGS
  cd piv${pb}_seq_tight_results_COMMON
  eval_COMMON_de generate-tst-COMMON_st.txt

  cd $MODEL_PATH
  python $FAIRSEQ_PATH/fairseq_cli/generate.py \
    --task speech_to_text --gen-subset tst-HE_st --pivot-beam $pb  --results-path piv${pb}_seq_tight_results_HE $MODEL_ARGS $EVAL_ARGS $OTHER_ARGS
  cd piv${pb}_seq_tight_results_HE
...
do
  ...
  echo "pivot-beam $pb"
  python $FAIRSEQ_PATH/fairseq_cli/generate.py \
    --task speech_to_text --gen-subset tst-COMMON_st --pivot-beam $pb --results-path piv${pb}_tok_tight_results_COMMON --ensemble-decoding --ensemble-decoding-weight-asr $MODEL_ARGS $EVAL_ARGS $OTHER_ARGS
  cd piv${pb}_tok_tight_results_COMMON
  eval_COMMON_de generate-tst-COMMON_st.txt

  cd $MODEL_PATH
  python $FAIRSEQ_PATH/fairseq_cli/generate.py \
    --task speech_to_text --gen-subset tst-HE_st --pivot-beam $pb  --results-path piv${pb}_tok_tight_results_HE --ensemble-decoding --ensemble-decoding-weight-asr $MODEL_ARGS $EVAL_ARGS $OTHER_ARGS
  cd piv${pb}_tok_tight_results_HE
  eval_HE_de generate-tst-HE_st.txt
done