...
MODEL_NAME=fair.trafo.ctc.r2l.pretrain-l2r.ft2021
...
--log-format simple \
--log-interval 100 \
--validate-save-subepochs 4 \
--finetune-from-model /work/smt4/gao/tran/speech_translation/final_models/mustc.en-de/asr.r2l.pretrain-l2r.best.pt
EOF
...
echo $DATA_PATH --task custom_speech_to_text --config-yaml $DATA_PATH/config_asr.i6approx.yaml $MODEL_ARGS $TRAIN_ARGS $OTHER_ARGS
echo '-------------------------'

if [ $NUM_GPUS -eq 1 ]; then
  WANDB_RUN_GROUP=$GROUP_NAME WANDB_NAME=$MODEL_NAME python3 $USER_DIR/cli/train.py \
    $DATA_PATH --task custom_speech_to_text --config-yaml $DATA_PATH/config_asr.i6approx.yaml $MODEL_ARGS $TRAIN_ARGS $OTHER_ARGS
else
