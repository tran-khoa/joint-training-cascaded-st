...
MODEL_NAME=asr.mustc.en-de.src6000.nocmvn
...
echo $DATA_PATH --task cached_speech_to_text --config-yaml /work/smt2/vtran/datasets/asr/parnia.mustc.asr.nofilter/config_asr_nocmvn.yaml $MODEL_ARGS $TRAIN_ARGS $OTHER_ARGS
echo '-------------------------'
...
WANDB_RUN_GROUP=$GROUP_NAME WANDB_NAME=$MODEL_NAME python /work/smt2/vtran/fairseq-st/projects/speech_translation/cli/train.py \
  $DATA_PATH --task cached_speech_to_text --config-yaml /work/smt2/vtran/datasets/asr/parnia.mustc.asr.nofilter/config_asr_nocmvn.yaml $MODEL_ARGS $TRAIN_ARGS $OTHER_ARGS
