...
MODEL_NAME=base.ctc.pretrain.nocmvn.embeddrop.init.noweightdecay
...
echo $DATA_PATH --task custom_speech_to_text --config-yaml $DATA_PATH/config_asr_nocmvn.yaml $MODEL_ARGS $TRAIN_ARGS $OTHER_ARGS
echo '-------------------------'

WANDB_RUN_GROUP=$GROUP_NAME WANDB_NAME=$MODEL_NAME python $USER_DIR/cli/train.py \
  $DATA_PATH --task custom_speech_to_text --config-yaml $DATA_PATH/config_asr_nocmvn.yaml $MODEL_ARGS $TRAIN_ARGS $OTHER_ARGS
