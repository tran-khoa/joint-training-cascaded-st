...
echo $(hostname) > CURRENT_MASTER
RESOLVED_MASTER=$(nslookup $(hostname) | tail -n2 | head -n1 | cut -d' ' -f2)

echo '-------------------------'
echo "This is the master script, running on $(hostname)"
echo "This resolves to $RESOLVED_MASTER"
echo '-------------------------'
