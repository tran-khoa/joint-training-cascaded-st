...
for temp in 0.1 0.5 2 4 8
do
  cd $MODEL_PATH
  echo "pivot-beam $pb"
  python $FAIRSEQ_PATH/fairseq_cli/generate.py \
    --task speech_to_text --gen-subset tst-COMMON_st --pivot-beam 4 --asr-eval-temp $temp --results-path temp"${temp}"_piv4_seq_results_COMMON $MODEL_ARGS $EVAL_ARGS $OTHER_ARGS
  cd piv${pb}_seq_results_COMMON
  eval_COMMON_de generate-tst-COMMON_st.txt

  cd $MODEL_PATH
  python $FAIRSEQ_PATH/fairseq_cli/generate.py \
    --task speech_to_text --gen-subset tst-HE_st --pivot-beam 4 --asr-eval-temp $temp --results-path temp"${temp}"_piv4_seq_results_HE $MODEL_ARGS $EVAL_ARGS $OTHER_ARGS
