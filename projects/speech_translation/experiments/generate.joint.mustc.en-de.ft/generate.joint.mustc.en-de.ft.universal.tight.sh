...
do
  ...
  python $FAIRSEQ_PATH/fairseq_cli/generate.py \
    --task speech_to_text --gen-subset tst-COMMON_st --pivot-beam $pb --results-path piv${pb}_seq_results_COMMON $MODEL_ARGS $EVAL_ARGS $OTHER_ARGS
  cd piv${pb}_seq_results_COMMON
  eval_COMMON_de generate-tst-COMMON_st.txt

  cd $MODEL_PATH
  python $FAIRSEQ_PATH/fairseq_cli/generate.py \
    --task speech_to_text --gen-subset tst-HE_st --pivot-beam $pb  --results-path piv${pb}_seq_results_HE $MODEL_ARGS $EVAL_ARGS $OTHER_ARGS
  cd piv${pb}_seq_results_HE
...
do
  ...
  echo "pivot-beam $pb"
  python $FAIRSEQ_PATH/fairseq_cli/generate.py \
    --task speech_to_text --gen-subset tst-COMMON_st --pivot-beam $pb --results-path piv${pb}_tok_results_COMMON --ensemble-decoding --ensemble-decoding-weight-asr $MODEL_ARGS $EVAL_ARGS $OTHER_ARGS
  cd piv${pb}_tok_results_COMMON
  eval_COMMON_de generate-tst-COMMON_st.txt

  cd $MODEL_PATH
  python $FAIRSEQ_PATH/fairseq_cli/generate.py \
    --task speech_to_text --gen-subset tst-HE_st --pivot-beam $pb  --results-path piv${pb}_tok_results_HE --ensemble-decoding --ensemble-decoding-weight-asr $MODEL_ARGS $EVAL_ARGS $OTHER_ARGS
  cd piv${pb}_tok_results_HE
  eval_HE_de generate-tst-HE_st.txt
done
...
cd $MODEL_PATH
python $FAIRSEQ_PATH/fairseq_cli/generate.py \
  --task speech_to_text --gen-subset tst-COMMON_st --pivot-beam 1 --results-path tight_results_COMMON --model-overrides "{'tight_integrated':True,'tight_integrated_decoding_temp':1.5}" $MODEL_ARGS $EVAL_ARGS $OTHER_ARGS
cd tight_results_COMMON
eval_COMMON_de generate-tst-COMMON_st.txt

cd $MODEL_PATH
python $FAIRSEQ_PATH/fairseq_cli/generate.py \
  --task speech_to_text --gen-subset tst-HE_st --pivot-beam 1 --results-path tight_results_HE --model-overrides "{'tight_integrated':True,'tight_integrated_decoding_temp':1.5}" $MODEL_ARGS $EVAL_ARGS $OTHER_ARGS
cd tight_results_HE
eval_HE_de generate-tst-HE_st.txt