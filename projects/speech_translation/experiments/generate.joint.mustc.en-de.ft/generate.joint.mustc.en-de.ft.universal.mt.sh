...
echo "Extracting MT checkpoint..."
python $FAIRSEQ_PATH/scripts/extract_model_from_cascade.py $MODEL_PATH/checkpoints/checkpoint_best.pt mt
echo "Done."
...
cat /work/smt4/gao/tran/speech_translation/datasets/text/out/tstCOMMON.en-de.32k.src-normalized/test.enc.en-de.en | \
  python /work/smt2/vtran/fairseq-st/fairseq_cli/interactive.py \
    $DATA_PATH --task custom_translation $EVAL_ARGS $OTHER_ARGS > $MODEL_PATH/mt/mt_gen_COMMON.txt
cd $MODEL_PATH/mt
/u/vtran/bin/eval_COMMON_de mt_gen_COMMON.txt

cd $MODEL_PATH
cat /work/smt4/gao/tran/speech_translation/datasets/text/out/tstHE.en-de.32k.src-normalized/test.enc.en-de.en | \
  python /work/smt2/vtran/fairseq-st/fairseq_cli/interactive.py \
    $DATA_PATH --task custom_translation $EVAL_ARGS $OTHER_ARGS > $MODEL_PATH/mt/mt_gen_HE.txt
cd $MODEL_PATH/mt
/u/vtran/bin/eval_HE_de mt_gen_HE.txt
